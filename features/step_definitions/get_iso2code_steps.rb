When('a client make a GET request to getiso2code endpoint with an existing iso2code') do
  @response = groupkt_get_iso2code('AF')
  @response_body = json(@response.body)
end

Then('GroupKT responds with a message informing the search was matched') do
  expect(@response.status).to eq 200
  expect(@response.headers['Content-Type']).to eq 'application/json;charset=UTF-8'
  fixture = open_file('../fixtures/get_iso2code_fixture.json.erb')
  options = { iso2code: 'AF' }
...
Then('GroupKT responds with the correct payload for each alpha2code') do
  ...
  # Converts the list of valid alpha3code and name into an array of strings
  alpha3code = list_of_alpha3codes.split("\n")
  name = list_of_names.split("\n")

  # For each alpha2code, match the result of get iso2code response with the expected result
  list_of_alpha2codes.split("\n").each_with_index do |alpha2code, index|
    response = groupkt_get_iso2code(alpha2code)
    response_body = json(response.body)
    options = { name: name[index], alpha2code: alpha2code, alpha3code: alpha3code[index] }
...
Then('GroupKT returns JSON charset=UTF-8 content regardless the Accept header sent on the request on iso2code API') do
  fixture = open_file('../fixtures/list_of_MIME_types.txt')
  file = open_to_write('../logs/log_each_content_type_iso2code_request')

  fixture.split("\n").each do |content_type|
    response = groupkt_get_iso2code('AF', accept: content_type)
...
When('a client make a POST request to getiso2code endpoint') do
  @response = groupkt_post_iso2code('AF')
end

When('a client make a PUT request to getiso2code endpoint') do
  @response = groupkt_put_iso2code('AF')
end

When('a client make a PATCH request to getiso2code endpoint') do
  @response = groupkt_patch_iso2code('AF')
end

Then('GroupKT responds with a message saying there were no matches for invalid input of iso2code')do
  response = groupkt_get_iso2code("1'%20or%20'1'%20=%20'1&password=1'%20or%20'1'%20=%20'1'")
...
Then('GroupKT fails gracefully when testing a list of user bad input data') do
  fixture = open_file('../fixtures/list_of_user_input_data.json')
  parsed_fixture = json(fixture)
  file = open_to_write('../logs/log_each_input_iso2code_request')

  parsed_fixture.each do |user_input|
    encoded_user_input = url_encode(user_input)

    response = groupkt_get_iso2code(encoded_user_input)
    expect(response.status).to_not eq 500
    expect{response}.to_not raise_error
